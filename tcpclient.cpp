...
void TcpClient::readMessage()
{
    ...
    ui->tcpClientStatuslabel->setText(tr("已接收 %1 MB (%2MB/s)""\n共%3MB.已用时：%4秒\n估计剩余时间：%5秒").arg(bytesReceive / (1024*1024)).arg(speed * 1000 / (1024 * 1024),0,'f',2).arg(TotalBytes / (1024*1024)).arg(useTime/(1000),0,'f',0).arg(TotalBytes/speed/1000 - useTime/1000,0,'f',0));
